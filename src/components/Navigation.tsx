...
const Navigation = () => {
  ...
  let ColorSite : any = useRef();
  let Links : any = useRef();
  let HamburgerLogo : any = useRef();
  let NavList : any = useRef();
  let NavLinks = gsap.utils.selector(Links);

  const { locale } = router;
  const language : any = locale === "fr" ? fr : en;
  ...
  const ActivatedHamburger = () => {
    ...
    (document.querySelector('body') as any).classList.toggle('scrollHide');
  }
